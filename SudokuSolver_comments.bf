...
[
    ...
    [ i1
        the subraction is done so that if its looking for a validity of a digit that has been fetched 
        it wont use digit's that were already used
        >>> >>> >>>  > i11
        [>>-<<-] subtract i11 from i13
        >> i13
        +++ +++ +++ increment 9 i13
        [
    ...
    save current cell number in cache (if needed):
    [
        >>> >>> >>>  >> i12
        [
            <<< <<< <<<  << i1
    ...
    fetch last cached cell (if needed):
    [
        >+ i2
        >>> >>> >>>  > i12
        [<<< <<< <<<  <->]
        ...
        [
            ...
            get the current number in the fetched cell
            [->+>>> >>> >>>  >>> >>+<<< <<< <<<  <<< <<<] copy i1 to i2 and i16
            > i2
            [-<+>] copy i2 to i1
            >>> >>> >>>
            ...
            decrementing i16 will cause the loop of finding empty cells not to run
            >- decrement i16
            <<< <<< <<<  <<< << i2
        ]
    ]
